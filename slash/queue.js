...
module.exports = {
    ...
    .addNumberOption((option) => option.setName("name").setDescription("Page number of the queue").setMinValue(1)),
        ...
        await interaction.editReply({
            embeds: [
                new EmbedBuilder()
                    ...
                    (currentSong ? `\` [${currentSong.duration}]\`${currentSong.title} -- <@${song.requestBy.id}>`: "none") + 
                    `\n\n**Queue\n${queueString()}`
                    )
                    .setFooter({
